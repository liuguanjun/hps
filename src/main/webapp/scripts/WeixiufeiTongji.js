...
function loadCaozuoyuan(node) {
	...
	// 基地没有选择时，不加载缴费年费下拉列表
	$.ajax({
		url: ctx + '/getWeixiufeiTongji/caozuoyuan/' + baseCode,
		type : "get",
		data : param,
		success: function(data) {
			$("#hpsUser_q").combobox({
...
//	});
    ...
    form.attr('action', ctx + '/report/maintain');
...
function searchTongjiInfo() {
	$('#data_grid').datagrid('options').url='${ctx}/getWeixiufeiTongji/weixiufeiTongji/{baseCode}';
